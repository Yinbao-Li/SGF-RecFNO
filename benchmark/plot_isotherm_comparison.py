#!/usr/bin/env python3
"""Three cases × six models: isotherm contour overlays vs. ground truth."""
import argparse
import os
import sys
...
from benchmark.visualize import plot_three_cases_isotherm_overlay
...
def parse_args():
    p = argparse.ArgumentParser(description='Plot isotherm contour overlays (3 cases × 6 models)')
    p.add_argument(
    ...
    p.add_argument('--out-name', default='three_cases_isotherm_overlay.png')
...
def main():
    ...
    for sample_idx in args.sample_idx:
        ...
        levels_k = gt_isotherm_levels_k(truth, DEFAULT_QUANTILES)
        print(f'  GT isotherm levels (K): {np.round(levels_k, 3)}', flush=True)
        chamfers = sorted(
            ((name, mean_chamfer_isotherms(pred, truth, levels_k=levels_k)) for name, pred in preds),
            key=lambda x: x[1],
        )
        for name, cd in chamfers:
            print(f'  {name:12s}  Chamfer = {cd:.3f} px', flush=True)
        cases.append({
    ...
    plot_three_cases_isotherm_overlay(cases, out_png, quantiles=DEFAULT_QUANTILES)
