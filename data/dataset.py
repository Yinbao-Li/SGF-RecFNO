...
from data.paths import DATA_ROOT, HEAT_H5, heat_field_key
...
class CylinderSNRDataset(Dataset):
    def __init__(self, index, mean=0, std=1, snr=False, SNRdB=20, test=False):
        ...
        df = open(f'{DATA_ROOT}/cylinder/Cy_Taira.pickle', 'rb')
...
class DarcyDataset(Dataset):
    def __init__(self, index, mean=0, std=0.01):
        ...
        self.mean, self.std = mean, std
        f = h5py.File(f'{DATA_ROOT}/darcy/darcy12000.h5', 'r')
...
class NOAADataset(Dataset):
    def __init__(self, index, mean=0, std=40):
        ...
        self.mean, self.std = mean, std
        data = h5py.File(f'{DATA_ROOT}/noaa/sst_weekly.mat')
        sst = data['sst'][:]
