#!/usr/bin/env python3
"""Quick SGF-RecFNO smoke run (5 epochs) with visualization."""
import glob
import logging
import os
import sys
...
from heat2D_sgf_recfno import build_model, compute_loss
...
def train_5_epochs():
    args = parse_sgf_args()
    args.exp = 'sgf_recfno_heat_5epoch'
    args.epochs = 5
    args.batch_size = 8
    args.plot_freq = 1
    args.val_interval = 1

    print('=== SGF-RecFNO 热传导训练 (5 epochs) ===', flush=True)
    ...
    for epoch in range(args.epochs):
        ...
        last_outputs, last_pre, last_coarse, last_sdf = None, None, None, None
        with torch.no_grad():
            for inputs, outputs in val_loader:
                ...
                last_pre = aux['field']
                last_coarse = aux['coarse']
                last_sdf = aux['sdf_self']
...
def visualize_test(args, history):
    ...
    with torch.no_grad():
        ...
        sdf_self = aux['sdf_self'][0].cpu().numpy()
    ...
    axes[0].imshow(target, cmap='coolwarm', origin='upper')
    axes[0].set_title('Truth')
    axes[1].imshow(pred, cmap='coolwarm', origin='upper')
    axes[1].set_title('SGF-RecFNO')
    axes[2].imshow(np.abs(err), cmap='viridis', origin='upper')
    axes[2].set_title('|Error|')
    axes[3].imshow(sdf_self.mean(axis=0), cmap='RdBu_r', origin='upper')
    axes[3].set_title('Self SDF (from coarse)')
    for ax in axes:
        ax.set_aspect('equal')
    plt.tight_layout()
    vis_path = os.path.join(out_dir, 'test_self_geometry.png')
    ...
    return ckpt_path, vis_path, hist_path, summary
